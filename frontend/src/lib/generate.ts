...
export async function generateGustoLedgerFile(file: File) {
  const body = new FormData();
  body.append("file", file);

  const response = await fetch(`${API_URL}/api/generate/gusto`, {
    method: "POST",
    body,
  });
...
export async function generateQboFile(file: File) {
  const body = new FormData();
  body.append("file", file);

  const response = await fetch(`${API_URL}/api/generate/qbo`, {
    method: "POST",
    body,
  });
