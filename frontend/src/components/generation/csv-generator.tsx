...
export function CsvGenerator() {
  ...
  async function handleGenerate() {
    ...
    try {
      const generated = await generateLedgerFile(file);
      setResult(generated);
    } catch (generateError) {
      setResult(null);
      setError(
  ...
  const included = result?.employees.filter((employee) => !employee.skipped) ?? [];
        ...
        <section className="overflow-hidden rounded-md border border-zinc-200 bg-white shadow-sm">
          <div className="border-b border-zinc-200 px-4 py-3">
            ...
            <p className="text-xs text-zinc-500">
              Check date {result.checkDate} · {result.filename}
            </p>
          </div>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {result.employees.map((employee) => (
                <TableRow key={`${employee.lastName}-${employee.firstName}`}>
                  <TableCell>
                  ...
                  <TableCell>
                    <Badge variant={employee.skipped ? "outline" : "secondary"}>
                      {employee.skipped
                        ? employee.skipReason ?? "skipped"
                        : "included"}
                    </Badge>
                  </TableCell>
                </TableRow>
